...
public final class EncryptUtils {
   ...
   * @return 密文
   */
  public static String encrypt3DESCode(String data, String key) {
    try {
      MessageDigest md5 = MessageDigest.getInstance(MD5);
      byte[] keyBytes = md5.digest(key.getBytes(DEFAULT_CHARSET));
      keyBytes = getKey(keyBytes);
      byte[] bytes =
          desTemplateCode(data.getBytes(DEFAULT_CHARSET), keyBytes, TripleDES_Algorithm, CIPHER_ALGORITHM, true);
  ...
  private static byte[] getKey(byte[] keyBytes) {
    ...
    if (keyBytes.length == 16) {
      keys = new byte[24];
      System.arraycopy(keyBytes, 0, keys, 0, 16);
      System.arraycopy(keyBytes, 0, keys, 16, 8);
    } else {
      keys = keyBytes;
    }
    return keys;
   ...
   * @return 密文
   */
  public static byte[] encrypt3DESCode(final byte[] data, final byte[] key) {
    return desTemplate(data, key, TripleDES_Algorithm, TripleDES_Transformation, true);
  }
   ...
   * @return 密文或者明文，适用于DES，3DES，AES
   */
  public static byte[] desTemplateCode(final byte[] data, final byte[] key, final String algorithm,
      final String transformation, final boolean isEncrypt) {
    if (data == null || data.length == 0 || key == null || key.length == 0) {
      return null;
    }
