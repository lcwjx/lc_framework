...
public final class LogUtils {
    ...
    if (!sTagIsSpace && !sLogHeadSwitch) {
      ...
      if (sLogHeadSwitch) {
        String tName = Thread.currentThread()
            .getName();
        final String head = new Formatter().format("%s, %s(%s:%d)", tName, targetElement.getMethodName(), fileName,
        ...
        if (sStackDeep <= 1) {
          ...
          for (int i = 1, len = consoleHead.length; i < len; ++i) {
            ...
            consoleHead[i] =
                new Formatter().format("%s%s(%s:%d)", space, targetElement.getMethodName(), targetElement.getFileName(),
                    targetElement.getLineNumber())
                    .toString();
          }
  ...
  public static class Config {
    private Config() {
      ...
      if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState()) && Utils.getApp()
          .getExternalCacheDir() != null) {
        sDefaultDir = Utils.getApp()
